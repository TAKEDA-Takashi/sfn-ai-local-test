...
### 🎯 新機能
- **AI生成プロンプトにCatch/Retryエラーハンドリングガイダンスを追加** - ステートマシンにCatch/Retryがある場合、エラーモックの書き方やテスト方法の具体例をプロンプトに自動挿入
- **AI生成プロンプトにVariables/Assignガイダンスを追加** - Assignフィールドのスコープルール、テスト時のvariables検証パターンなど詳細なガイダンスを提供

### ✨ 改善
- **デフォルトAIモデルをClaude Sonnet 4.6に更新** - 最新モデルによる生成品質の向上
- **Direct APIパスの品質改善** - system/userプロンプト分離、temperature:0、max_tokens:16384の適用
- **npmからpnpmへパッケージマネージャ移行** - より高速で効率的な依存関係管理

### 🐛 バグ修正
- **stateExecution.inputの記録修正** - Parameters処理前の生入力を正しく記録するよう修正

### ♻️ リファクタリング
- **State型をDiscriminated Unionに変更** - 906行のクラス定義を削除し、軽量な型定義+型ガード関数に置換
- **PromptBuilderを7モジュールに分割** - 1422行→220行+7モジュール（prompt-sections/）に分割
- **StateMachineValidator責務分離** - 854行→520行、ステート探索の統一、ヘルパーモジュール抽出
- **CLI重複ロジック統一** - generate.ts、run.ts、レポーターの重複コードを共通化
- **CDK抽出関数統合** - 重複していたCDK関連ユーティリティを統合
- **未使用コード除去** - knipによるデッドコード検出・除去、未使用依存関係の削除
